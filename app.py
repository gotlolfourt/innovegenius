...
def otp_store():
    ...
    if not app_id or not otp_hash:
        return jsonify({'error': 'Missing app_id or otp_hash'}), 400
...
def otp_verify():
    ...
    if not app_row or not app_row['otp_hash']:
        con.close()
        return jsonify({'error': 'No OTP found for this application. Please request OTP first.'}), 400
